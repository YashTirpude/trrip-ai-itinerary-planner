...
import {
  MapPin, Calendar, Share2, Check, Clock,
  Plane, Hotel, Coffee, Train, Bus, Activity, ArrowLeft
} from "lucide-react";
...
export default function ItineraryPage() {
  ...
  if (!itinerary) return (
    <div className={styles.page}>
      <Navbar />
      <div className={styles.loadingWrap}><p>Itinerary not found.</p></div>
    </div>
  );
  ...
  return (
    <div className={styles.page}>
      ...
      <main className={styles.main}>
        {/* Back */}
        <Link to="/dashboard" className={styles.back}><ArrowLeft size={16} /> My Trips</Link>
          ...
          <div>
            ...
            {itinerary.startDate && itinerary.endDate && (
              <p className={styles.dates}>
                ...
                <span className={styles.badge}>{itinerary.days?.length || 0} days</span>
              </p>
            )}
          </div>
          <button className={`${styles.shareBtn} ${copied ? styles.copied : ""}`} onClick={copyShareLink}>
            ...
            {day && (
              <div className={styles.dayContent} key={activeDay}>
                <div className={styles.dayHeader}>
                  <h2>{day.title}</h2>
                  {day.date && <span className={styles.fullDate}>{format(new Date(day.date), "EEEE, MMMM d")}</span>}
                </div>
                  ...
                  {day.activities?.map((act, i) => (
                    <div key={i} className={styles.activity}>
                      ...
                      <div className={styles.actCard}>
                        ...
                        {act.location && (
                          <span className={styles.actLocation}><MapPin size={12} /> {act.location}</span>
                        )}
                      </div>
                    </div>
                  ))}
